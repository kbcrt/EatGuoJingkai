...
(function(w) {
    ...
    function updatePanel() {
        if (mode === MODE_NORMAL) {
            if (_gameTimeNum <= 0) {
                GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;Time out！';
                gameOver();
    ...
    function shareText(score) {
        ...
        if (score <= 49) return '郭敬锴对着你竖起了国际友好手势';
        if (score <= 99) return '郭敬锴说你真是逊啦';
        if (score <= 149) return '您';
        if (score <= 199) return '郭敬锴妈妈问他为什么要跪着';
        return '您';
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "吃掉郭敬锴";
        if (cookie("keyboard")) {
